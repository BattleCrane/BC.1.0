package PolyBot.PolyCombinations.IteratorBuilding;
...
public class PolyIteratorBuilder {
    ...
    public void findCombination(BattleManager battleManager, int howICanBuild) {
        List<EstimatedUnit> estimatedUnitList = Arrays.asList(
                //Barracks:
                new EstimatedUnit(battleManager, battleManager.getBarracks(), (s) -> {
                }, (e) -> {
                }) {
                    @Override
        ...
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j++) {
                ...
                if (currentUnity.substring(1).equals("    0")) { //Если пустая клетка
                    for (EstimatedUnit unit : estimatedUnitList) {
    ...
    private void checkConditionalUnit(BattleManager battleManager, EstimatedUnit estimatedUnit, Point point, String currentUnity, int howICanBuild) {
        if (estimatedUnit.isPerformedCondition(point)) {//Если территория свободна и рядом есть мои строения
            PriorityUnit priorityUnit = new PolyMainProbe().probeBuildingTest(battleManager, estimatedUnit.getUnity(), point); //Исследуем приоритет на бараки
