        ...
        if (step < mergedList.getUnits().size()) {
            ...
            if (!estimatedUnit.isPerformedCondition(p.getPoint())) { //Если условия не выполняются, тогда рассматриваем альтернативный вариант строения и улучшения
                ...
                if (lastCreated.getPriority() > currentUpgraded.getPriority()){
                    ...
                    methodStart = o -> battleManager.upgradeBuilding(o.getPoint(), battleManager.getPlayer());
...
//                        case "t":
//                            PolyRadiusProbe.RadiusParams radiusParams = new PolyRadiusProbe
//                                    .RadiusParams(battleManager.getTurret(),  point);
//                            priorityUnit = (PriorityUnit) radiusProbe.probe(radiusParams);
//                            break;
//                        case "w":
